...
fn construct_hnsw(data: Arc<dyn DatasetT>, params: HnswParams, seed: u64) -> SliceHnsw {
    ...
    let (mut layers, insert_positions) =
        make_layers_with_empty_neighbors(data.len(), params, &mut rng);
    // println!("insert positions:");
    // for (
    //     id,
    //     &InsertPosition {
    //         level,
    //         idx_in_layer,
    //     },
    // ) in insert_positions.iter().enumerate()
    // {
    //     println!("  id {id} : level {level}  idx {idx_in_layer} ");
    // }
    ...
    for id in 1..data.len() {
        ...
        insert_element(&mut ctx, id, pos.level, pos.idx_in_layer);
    }
...
fn insert_element(
    ...
    // println!("insert id: {id}, insert_level: {insert_level}, idx_in_layer: {idx_in_layer}, top_level: {}", ctx.top_level);
    ...
    for l in (ctx.top_level + 1..=insert_level).rev() {
        // println!(
        //     "    level {l}: {idx_in_layer} -> {}",
        //     ctx.layers[l].entries[idx_in_layer].lower_level_idx
        // );
    ...
    for l in (0..=ctx.top_level.min(insert_level)).rev() {
        ...
        // println!(
        //     "    level {l}: {idx_in_layer} -> {}",
        //     layer.entries[idx_in_layer].lower_level_idx
        // );
