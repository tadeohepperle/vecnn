...
class ModelParams:
    kind: str
    seed: str
    def __init__(self, kind: str, seed: int = 42, **kwargs) -> None:
        self.kind = kind
        self.seed = seed
...
class Model:
    ...
    def __init__(self, data: np.ndarray, params: ModelParams):
        ...
        if params.kind == 'vecnn_vptree':
            ...
            dataset = vecnn.Dataset(data) # not ideal
            start = time.time()
            hnsw = vecnn.build_hnsw_by_transition(dataset, params.max_chunk_size, params.same_chunk_m_max, params.m_max, params.neg_fraction, params.keep_fraction, params.distance, params.seed)
            build_time = time.time() - start
            self.build_metrics =  BuildMetrics(build_time=build_time, num_distance_calculations=hnsw.num_distance_calculations_in_build)
            self.vecnn_hsnw = hnsw
