...
class TenMExperiments:
    ...
    def recall_greater_80(self) -> pd.DataFrame:
        return self.recall_gt_80[exp.recall_gt_80["recall"] > 0.8][["ef","recall","search_ms", "build_secs", "params", "threaded"]]
    ...
    def recall_greater_80_compare(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(objs_list, columns=["params", "both", "threaded_build_secs", "threaded_search_ms", "threaded_recall", "single_build_secs", "single_search_ms", "single_recall"])
...
df_print(exp.recall_greater_80_compare())
...
params_and_ef_filtered = exp.params_and_ef[exp.params_and_ef["params"].isin(crossing_80)]
...
#     plt.plot(subdf["ef"], subdf["search_recall"],  color=MODEL_KIND_COLOR[kind])
#     plt.scatter(subdf["ef"], subdf["search_recall"], label=kind, color=MODEL_KIND_COLOR[kind])
# plt.xlabel("ef")
# plt.ylabel("recall")
...
# print(exp.recall_gt_80[["threaded", "model_kind", "build_secs", "search_ms"]])

# for kind in ["Ensemble", "RNNGraph", "Hnsw"]:
#     subdf = exp.recall_gt_80[(exp.recall_gt_80["model_kind"] == kind) & (exp.recall_gt_80["threaded"] == False)]
